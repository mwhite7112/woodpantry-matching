...
PR CI runs:
- Blocking lint using `.golangci.yaml`
- Advisory lint using `.golangci-advisory.yaml` (non-blocking via `continue-on-error`)
- Unit tests: `go test -race ./...`
- Integration-tag sweep: `go test -race -tags=integration ./...` (currently no integration-tagged tests)
- Docker build check: `docker build --tag woodpantry-matching:ci .`
